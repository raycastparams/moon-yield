...
local iyassets = {
	...
	["infiniteyield/assets/logo.png"] = "rbxassetid://135499690817973",
...
if makefolder and isfolder and writefile and isfile then
	pcall(function() -- good executor trust
		...
		for _, folder in {"infiniteyield", "infiniteyield/assets", "infiniteyield/moonyield", "infiniteyield/moonyield/cache", "infiniteyield/moonyield/info"} do
		...
		if IsOnMobile then writefile("infiniteyield/assets/.nomedia", ""); writefile("infiniteyield/moonyield/.nomedia", "") end
...
AliasHint.Font = Enum.Font.Montserrat
...
PluginsHint.Font = Enum.Font.Montserrat
...
PositionsHint.Font = Enum.Font.Montserrat
...
Path.Font = Enum.Font.Montserrat
Path.TextSize = 14
Path.Text = ""
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			...
			inviteButton.Text = "Copied official IY discord server" -- im not bothered to make a moon yield dc server yk
		else
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Moon Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
local UNCROOTURL = "https://raycastparams.github.io/resources/"

local function loadunc(url)
    local ll = UNCROOTURL .. url
...
CMDs[#CMDs + 1] = {NAME = 'hydroxide / hspy', DESC = 'Loads Hydroxide for Mobile and PC'}
...
addcmd('serverpositionpredictor', {'spp'}, function(args, speaker)
...
addcmd('keepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		notify('KeepIY','Moon Yield will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		notify('KeepIY','Moon Yield will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
...
addcmd('hydroxide',{'hspy'},function(args, speaker)
	if IsOnMobile then
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet("https://raycastparams.github.io/moon-yield/version")
		return HttpService:JSONDecode(versionJson)
	end)

	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify("Outdated", "Get the new version on GitHub")
		end
